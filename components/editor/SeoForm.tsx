...
const SEOForm: FC<Props> = ({ initialValue, title = '', onChange }): JSX.Element => {
  ...
  return (
    <div className="space-y-4 ">
      ...
      <Input value={slug} onChange={handleChange} name="slug" placeholder="slug-goes-here" label="Slug:" />
      <Input value={tags} onChange={handleChange} name="tags" placeholder="React, Next JS" label="Tags:" />
...
const Input: FC<{
  ...
  return (
    <label className="block relative">
      ...
      <input type="text" name={name} value={value} placeholder={placeholder} className={classNames(commonInput, 'italic pl-12')} onChange={onChange} />
    </label>
  );
};
